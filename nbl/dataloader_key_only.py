...
class NBLNxDataset(object):
    ...
    def process(self):
        ...
        self.keys = []
        error_instance = []
        bar = tqdm.tqdm(enumerate(all_keys))
        ...
        for i, key in bar:
            try:
                ...
                pkl.dump(
                    nx_g, open(os.path.join(self.save_dir, f'nx_{i}'), 'wb')
                )
            except ParseError:
                err_count += 1
                ...
                json.dump(error_instance, open('error_instance_nbl.json', 'w'))
                continue
...
class NBLFullDGLDataset(DGLDataset):
    def __init__(self, raw_dir=ConfigClass.nbl_raw_dir,
        ...
        self.nx_dataset = NBLNxDataset(raw_dir, save_dir)
...
class NBLASTDGLDataset(DGLDataset):
    def __init__(self, raw_dir=ConfigClass.nbl_raw_dir,
                 save_dir=ConfigClass.preprocess_dir_nbl):
        self.graph_save_path = os.path.join(
