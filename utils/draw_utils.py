...
def cfg_to_agraph(cfg: nx.MultiDiGraph, path: str):
    ...
    nx.drawing.nx_agraph.to_agraph(cfg).draw(path)
...
def ast_to_agraph(ast: nx.MultiDiGraph, path: str):
    ...
    nx.drawing.nx_agraph.to_agraph(ast).draw(path)
...
def cfg_ast_to_agraph(cfg_ast: nx.MultiDiGraph, path: str):
    for node in cfg_ast.nodes():
        ...
        elif cfg_ast.nodes['node']['graph'] == 'graph':
            cfg_ast.nodes[node]['label'] = cfg_ast.nodes[node]['ntype'] + \
                ' ' + cfg_ast.nodes[node]['token']
    nx.drawing.nx_agraph.to_agraph(cfg_ast).draw(path)


def g_to_agraph(cfg_ast_cov: nx.MultiDiGraph, path):
    g = cfg_ast_cov
    for node in g.nodes():
        ...
        elif g.nodes['node']['graph'] == 'graph':
            g.nodes[node]['label'] = g.nodes[node]['ntype'] + \
                ' ' + g.nodes[node]['token']
        else:
            g.nodes[node]['label'] = g.nodes[node]['name']

    nx.drawing.nx_agraph.to_agraph(g).draw(path)
