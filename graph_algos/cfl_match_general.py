...
def extend_cpi_top_down(last_q: nx.MultiDiGraph, q: nx.MultiDiGraph,
    ...
    TODO: implement NOTE: STIll UNSTABLE
    Parameters
    ----------
    q : nx.MultiDiGraph
        The source graph to be used for matching,
        has some overlapping with last_q
        note: at least root has to be visited in last_q
    G : nx.MultiDiGraph
        The target graph to be matched against
    root: str
        The anchored chosen root node, can be random if needed
    ...
    for node in q.nodes():
        q.nodes[node]['visited'] = False
        q.nodes[node]['prev_visited'] = False
    ...
    for lev in range(check_visited, n_levels):
        ...
        n_modifieds = []
        for u in l2n_map[lev]:
            # If it's already visited from last q
            cnt_dict = create_empty_cnt_dict(edge_labels)
            u_is, u_os, u_as = all_neighbors(u, q)
            # Only process if it has neighbor in newly added nodes
        ...
        # Backward candidate pruning
        for u in l2n_map[lev][::-1]:
            if q.nodes[u]['prev_visited'] and u not in n_modifieds:
                continue
            cnt_dict = create_empty_cnt_dict(edge_labels)
            for io_switch in q.nodes[u]['UN']:
                check_edge_func = check_edge_in if io_switch == 'i' else check_edge_out
...
def extend_cpi_bottom_up(q: nx.MultiGraph, G: nx.MultiDiGraph,
                         node_dict: Dict[str, TreeNode],
                         check_label_func):
    ''' extend CPI bottom up with new nodes candidate pruning '''
    edge_labels = get_all_labels(q, G)
    l2n_map = defaultdict(list)
    for node in node_dict:
        lev = q.nodes[node]['distance']
...
def cpi_top_down_node_only(q: nx.MultiDiGraph, G: nx.MultiDiGraph,
                           root: str, node_dict: Dict[str, TreeNode],
                           check_label_func=lambda u, q, v, G:
                           q.nodes['u']['label'] == G.nodes['v']['label']):
    ''' Building CPI top-down but dont construct edge dict between cpis
    Parameters
    ----------
    q : nx.MultiDiGraph
        The source graph to be used for matching
...
def cpi_bottom_up_node_only(q: nx.MultiGraph, G: nx.MultiDiGraph,
                            node_dict: Dict[str, TreeNode],
                            check_label_func):
    ''' CPI bottom up candidate pruning
      Parameters
...
def extend_cpi(last_q, q, G, check_label_func, root_name='n0'):
    q = q.copy()
    G = G.copy()
    roots, node_dicts = graph2spanning_trees(q, root_name)
    for root, node_dict in zip(roots, node_dicts):
        node_dict, q = extend_cpi_top_down(
            last_q, q, G, root.node_label,
            node_dict, check_label_func)
        node_dict, q = extend_cpi_bottom_up(q, G, node_dict,
                                            check_label_func)
    return node_dicts, q


def build_cpi_node_only(q, G, check_label_func, root_name='n0'):
    q = q.copy()
    G = G.copy()
    roots, node_dicts = graph2spanning_trees(q, root_name)
    for root, node_dict in zip(roots, node_dicts):
        node_dict, q = cpi_top_down_node_only(q, G, root.node_label,
                                              node_dict, check_label_func)
        node_dict, q = cpi_bottom_up_node_only(q, G, node_dict,
                                               check_label_func)
    return node_dicts, q
