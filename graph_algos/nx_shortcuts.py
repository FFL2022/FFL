...
def where_node(*filter_funcs, **kwargs):
    ...
    lambdas = [
        (lambda nx_g, x: nx_g.nodes[x][k] == v) if not isinstance(v, list) else
        (lambda nx_g, x: nx_g.nodes[x][k] in v) for k, v in kwargs.items()
    ]
    lambda_f = lambda x: all(f(x) for f in filter_funcs)
    lambda_final = lambda nx_g, x: all(l(nx_g, x)
                                       for l in lambdas) and lambda_f(x)
    return lambda_final
...
def where_node_not(*filter_funcs, **kwargs):
    lambdas = [
        (lambda nx_g, x: nx_g.nodes[x][k] == v) if not isinstance(v, list) else
        (lambda nx_g, x: nx_g.nodes[x][k] not in v) for k, v in kwargs.items()
    ]
    lambda_f = lambda x: not any(f(x) for f in filter_funcs)
    lambda_final = lambda nx_g, x: (not any(l(nx_g, x)
                                            for l in lambdas)) and lambda_f(x)
    return lambda_final
...
def where_edge(where_source=None, where_target=None, *filter_funcs, **kwargs):
    ...
    lambdas_single_filter = lambda u, v, e: all(
        filter_func(u, v, e) for filter_func in filter_funcs)
    lambdas_multi_filter = lambda u, v, k, e: all(
        filter_func(u, v, k, e) for filter_func in filter_funcs)
    lambda_final_single = lambda nx_g, u, v, e: (where_source(
        nx_g, u) and where_target(nx_g, v) and lambdas_single_filter(
            u, v, e) and all(l(e) for l in lambdas_e))
    lambda_final_multi = lambda nx_g, u, v, k, e: (where_source(
        nx_g, u) and where_target(nx_g, v) and lambdas_multi_filter(
            u, v, k, e) and all(l(e) for l in lambdas_e))
    lambda_final = lambda nx_g, x: lambda_final_multi(nx_g, *x) if isinstance(
        nx_g, nx.MultiGraph) else lambda_final_single(nx_g, *x)
    return lambda_final
...
def edges_where(nx_g, where_source, where_target, *filter_funcs, **kwargs):
    ...
    if isinstance(nx_g, nx.MultiGraph):
        ...
        return [(u, v, e) for u, v, e in nx_g.edges(data=True)
                if l(nx_g, (u, v, e))]
...
def neighbors_in(u, q, filter_func=None):
    ...
    if filter_func is None:
        ...
        return list(
            set(
                list(u_n
                     for u_n, _, k, e in q.in_edges(u, data=True, keys=True)
...
def neighbors_out(u, q, filter_func=None):
    ...
    if filter_func is None:
        ...
        return list(
            set(
                list(u_n
                     for _, u_n, k, e in q.out_edges(u, data=True, keys=True)
