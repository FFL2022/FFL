...
class MPNN(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_index, weights=None):
        if weights is None:
            ...
            orig_output = torch.autograd.Variable((self.lin_tgt(x_i) + self.lin_src(x_j) + self.emb_self_edge((edge_index[1] == edge_index[0]).long())), requires_grad=False)
            output = weights * orig_output
            print(weights.requires_grad, output.requires_grad)
            return output
